...
class Environment:
    def __init__(self, all_cooked_time, all_cooked_bw, random_seed=RANDOM_SEED, handover='MEA'):
        ...
        self.cur_sat_id = self.get_best_sat_id(self.mahimahi_ptr - 1)
    ...
    def get_video_chunk(self, quality):
        ...
        while True:  # download video chunk over mahimahi
            ...
            if throughput == 0.0:
                ...
                self.cur_sat_id = self.get_best_sat_id(self.mahimahi_ptr)
                self.available_sat_list = self.get_available_sats_id()
    ...
    def get_best_sat_id(self, mahimahi_ptr=None):
        if self.handover == 'MVT':
            return self.get_best_sat_id_MVT(mahimahi_ptr)
        elif self.handover == 'MRA':
            return self.get_best_sat_id_MRA(mahimahi_ptr)
        else:
            return self.get_best_sat_id_MRA(mahimahi_ptr)
            

    def get_best_sat_id_MRA(self, mahimahi_ptr=None):
        best_sat_id = None
        best_sat_bw = 0
    ...
    def get_best_sat_id_MVT(self, mahimahi_ptr=None):
        best_sat_id = None
        best_sat_remain = 0
        ...
        for sat_id, sat_remain in self.cooked_remain.items():
            if best_sat_remain < sat_remain[mahimahi_ptr]:
                best_sat_id = sat_id
                best_sat_remain = sat_remain[mahimahi_ptr]
