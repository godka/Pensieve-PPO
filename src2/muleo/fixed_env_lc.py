...
class Environment:
            ...
            for sat_id, sat_bw in self.all_cooked_bw[trace_idx].items():
                ...
                for index in range(len(sat_bw)):
                    ...
                    while index + count < len(sat_bw) and sat_bw[index] != 0:
                        count += 1
                    self.all_cooked_remain[trace_idx][sat_id].append(count)        # pick a random trace file
        self.trace_idx = 0
    ...
    def get_best_sat_id(self, agent, mahimahi_ptr=None):
        ...
        for sat_id, sat_bw in self.cooked_bw.items():
            ...
            bw = sum(bw_list) / len(bw_list)
            if best_sat_bw < bw:
                if self.connection[sat_id][mahimahi_ptr] == -1 or self.connection[sat_id][mahimahi_ptr + 1] == agent:
                    best_sat_id = sat_id
                    best_sat_bw = bw
