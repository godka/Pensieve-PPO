...
class Environment:
    ...
    def get_video_chunk(self, quality, agent, model_type):
        ...
        if self.video_chunk_counter[agent] >= TOTAL_VIDEO_CHUNCK or end_of_network or self.mahimahi_ptr[agent] >= len(self.cooked_bw[self.cur_sat_id[agent]]):
    ...
    def reset(self):
        ...
        self.mahimahi_ptr = [0 for _ in range(self.num_agents)]
        self.last_mahimahi_time = [self.cooked_time[self.mahimahi_start_ptr - 1] for _ in range(self.num_agents)]
                ...
                if self.cooked_bw[next_sat_id][self.mahimahi_ptr[agent] - 1] != 0.0 and self.cooked_bw[next_sat_id][self.mahimahi_ptr[agent]] != 0.0:
                    ...
                    if method == "harmonic-mean":
                        for i in range(-5, 0, 1):
                            if self.cooked_bw[next_sat_id][self.mahimahi_ptr[agent]+i - 1] != 0:
                                self.predict_bw(next_sat_id, agent, robustness, mahimahi_ptr=self.mahimahi_ptr[agent]+i, plus=True)

                            if self.cooked_bw[self.cur_sat_id[agent]][self.mahimahi_ptr[agent] + i - 1] != 0:
    ...
    def get_runner_up_sat_id(self, agent, method="holt-winter"):
        ...
        for sat_id, sat_bw in self.cooked_bw.items():
            ...
            if sat_id == self.cur_sat_id[agent]:
                continue
            if self.cooked_bw[sat_id][self.mahimahi_ptr[agent] - 1] == 0:
                continue
    ...
    def predict_bw(self, sat_id, agent, robustness=True, plus=False, mahimahi_ptr=None):
        ...
        if num_of_user_sat == 0:
            ...
            try:
                ...
                print(mahimahi_ptr - 1)
                print(self.cooked_bw[sat_id][mahimahi_ptr-5:mahimahi_ptr-2])
                print(len(self.cooked_bw[sat_id]))

        if past_bw == 0:
            # not possible
            print(mahimahi_ptr - 1)
            print(self.cooked_bw[sat_id][mahimahi_ptr - 5:mahimahi_ptr+1])
            print(len(self.cooked_bw[sat_id]))
            assert past_bw != 0
