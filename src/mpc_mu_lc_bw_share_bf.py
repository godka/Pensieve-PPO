import numpy as np
from muleo_lc_bw_share_beamform import load_trace
from muleo_lc_bw_share_beamform import fixed_env as env
...
def main():
    ...
    all_cooked_time, all_cooked_bw, all_file_names = load_trace.load_trace(TEST_TRACES, split_condition="test")
    num_of_users = len(list(all_cooked_bw[0].keys()))
    ...
    for idx, ax in enumerate(axs):
        ...
        ax.plot(tmp_time_stamp, tmp_rewards)
        ax.set_title('Network Bandwidth')
        ax.set_ylabel('Satellite ' + str(idx))
    f.tight_layout()
    plt.show()
        ...
        if agent is not None:
            ...
            log_file.write("{: <15} {: <10} {: <10} {: <15} {: <15} {: <15}"
                           ...
                           .format(str(round(time_stamp[agent] / M_IN_K, 3)), str(agent),
                                   ...
                                   str(round(video_chunk_size, 3)), str(round(delay, 3)), str(round(reward, 3)), str(cur_sat_id), str(is_handover),
                                   str(float(video_chunk_size) / float(delay) / M_IN_K), str(sat_status)))
            log_file.flush()
