...
from util.constants import EPSILON, MAX_RATIO, MPC_FUTURE_CHUNK_COUNT, QUALITY_FACTOR, REBUF_PENALTY, SMOOTH_PENALTY, MPC_PAST_CHUNK_COUNT
...
class Environment:
    ...
    def get_video_chunk(self, quality, agent, model_type, runner_up_sat_id, ho_stamp, do_mpc=False):
        ...
        if self.video_chunk_counter[agent] >= TOTAL_VIDEO_CHUNCK or end_of_network:
            ...
            self.cur_satellite[self.cur_sat_id[agent]].remove_ue(agent)
        ...
        # num of users
        cur_sat_user_num = len(self.cur_satellite[self.cur_sat_id[agent]].get_ue_list())
        ...
        for idx, bitrate_level in enumerate(quality):
            ...
            if ho_stamp == idx:
                ...
                runner_up_sat_id, _ = self.get_runner_up_sat_id(agent, mahimahi_ptr=mahimahi_ptr,
                                                                method="harmonic-mean", plus=True)
    ...
    def get_next_sat_info(self, agent, mahimahi_ptr=None):
        ...
        for i in range(MPC_PAST_CHUNK_COUNT, 1, -1):
            if mahimahi_ptr - i >= 0:
                if len(self.cur_satellite[self.cur_sat_id[agent]].get_ue_list()) == 0:
                    bw_list.append(sat_bw[mahimahi_ptr - i])
                else:
                    bw_list.append(sat_bw[mahimahi_ptr - i] / len(self.cur_satellite[self.cur_sat_id[agent]].get_ue_list()))
        if len(bw_list) == 0:
            bw = 0
        else:
        ...
        for sat_id, sat_bw in self.cooked_bw.items():
            ...
            for i in range(MPC_PAST_CHUNK_COUNT, 1, -1):
                if mahimahi_ptr - i >= 0 and sat_bw[mahimahi_ptr - i] != 0:
                    if len(self.cur_satellite[sat_id].get_ue_list()) == 0:
                        bw_list.append(sat_bw[mahimahi_ptr - i])
                    else:
                        bw_list.append(sat_bw[mahimahi_ptr - i] / (len(self.cur_satellite[sat_id].get_ue_list()) + 1))
            if len(bw_list) == 0:
                continue
            bw = sum(bw_list) / len(bw_list)
            if best_sat_bw < bw:
    ...
    def get_best_sat_id(self, agent, mahimahi_ptr=None):
        ...
        for sat_id, sat_bw in self.cooked_bw.items():
            if len(self.cur_satellite[sat_id].get_ue_list()) == 0:
                ...
                real_sat_bw = sat_bw[mahimahi_ptr] / (len(self.cur_satellite[sat_id].get_ue_list()) + 1)
    ...
    def calculate_mpc_with_handover_exhaustive_reduced(self, agent):
        ...
        cur_sat_ids = [self.cur_sat_id[i] for i in range(self.num_agents)]
        runner_up_sat_ids = [self.get_runner_up_sat_id(i, method="harmonic-mean", plus=True)[0] for i in range(self.num_agents)]
        ...
        for full_combo in chunk_combo_option:
            ...
            if np.nanmean(rewards) > np.nanmean(max_rewards):
                ...
                max_rewards = rewards
                # ho_stamps = ho_positions
            elif np.nanmean(rewards) == np.nanmean(max_rewards) and np.nansum(combos) >= np.nansum(
                    best_combos):
                # elif np.nanmean(rewards) == np.nanmean(max_rewards) \
                #         and (rewards[agent] >= max_rewards[agent] or combos[agent][0] >= best_combos[agent][0]):
                best_combos = combos
            ...
            if op_vars:
                ...
                ue_ratio = minimize(
                    ...
                    args=(combos, cur_sat_ids, runner_up_sat_ids, sat_user_nums,
                          ...
                          None, user_info, bw_ratio, best_bws),
                    constraints=constraints,
                    bounds=bounds,
                    method="SLSQP"  # or BFGS
                )
        ...
        for next_sat_id, next_sat_bw in self.cooked_bw.items():
            ...
            if next_sat_id == self.cur_sat_id[agent]:
                ...
                if self.cooked_bw[next_sat_id][self.mahimahi_ptr[agent] - 1] != 0.0 and self.cooked_bw[next_sat_id][
                    ...
                    if method == "harmonic-mean":
                        tmp_next_bw = self.predict_bw(next_sat_id, agent, robustness, mahimahi_ptr=self.mahimahi_ptr[agent],
                                        plus=False, past_len=MPC_PAST_CHUNK_COUNT)
                        tmp_cur_bw = self.predict_bw(self.cur_sat_id[agent], agent, robustness,
                                        mahimahi_ptr=self.mahimahi_ptr[agent], plus=False, past_len=MPC_PAST_CHUNK_COUNT)
    ...
    def objective_function(self, x, combos, cur_sat_ids, runner_up_sat_ids, sat_user_nums,
        ...
        return curr_rebuffer_time # * 100 - total_buffer_diff / 10
    ...
    def get_runner_up_sat_id(self, agent, method="holt-winter", mahimahi_ptr=None, plus=True):
        best_sat_id = self.cur_sat_id[agent]
        best_sat_bw = 0
        if mahimahi_ptr is None:
            mahimahi_ptr = self.mahimahi_ptr[agent]
    ...
    def predict_bw(self, sat_id, agent, robustness=True, plus=False, mahimahi_ptr=None, past_len=None):
        ...
        if mahimahi_ptr <= 0:
            return self.cur_satellite[sat_id].data_rate(self.cur_user[agent], 0, plus)
        ...
        past_bw = self.cur_satellite[sat_id].data_rate(self.cur_user[agent], mahimahi_ptr-1, plus)

        if past_bw == 0:
            return self.cur_satellite[sat_id].data_rate(self.cur_user[agent], mahimahi_ptr, plus)
        ...
        for index in range(start_index, mahimahi_ptr):
            past_bws.append(self.cur_satellite[sat_id].data_rate(self.cur_user[agent], index, plus))
    ...
    def get_num_of_user_sat(self, sat_id):
        ...
        if sat_id == "all":
            ...
            for tmp_sat_id in self.cur_satellite.keys():
                if len(self.cur_satellite[tmp_sat_id].get_ue_list()) != 0:
                    filtered_num_of_user_sat[tmp_sat_id] = len(self.cur_satellite[tmp_sat_id].get_ue_list())
            return filtered_num_of_user_sat
        if sat_id in self.cur_satellite.keys():
            return len(self.cur_satellite[sat_id].get_ue_list())
