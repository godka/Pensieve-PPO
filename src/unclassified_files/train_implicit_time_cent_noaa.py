...
from env.multi_bw_share.env_cent_noaa_time import ABREnv
from models.rl_multi_bw_share.ppo_spec import ppo_cent_his as network
import tensorflow.compat.v1 as tf
import structlog
import logging

from util.constants import A_DIM, NUM_AGENTS, MAX_SAT, PAST_SAT_LOG_LEN, TRAIN_NOAA_TRACES
...
RANDOM_SEED = 42
SUMMARY_DIR = './ppo_imp_cent_noaa'
MODEL_DIR = '..'
TEST_LOG_FOLDER = './test_results_imp_cent_noaa'
PPO_TRAINING_EPO = 5
...
def testing(epoch, nn_model, log_file):
    ...
    # run test script
    log.info('python test_implicit_time_cent_noaa.py ', nn_model=nn_model + ' ' + str(USERS))
    os.system('python test_implicit_time_cent_noaa.py ' + nn_model + ' ' + str(USERS))
