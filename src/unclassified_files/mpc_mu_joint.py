import numpy as np
from unclassified_files.muleo_joint import load_trace
from unclassified_files.muleo_joint import fixed_env as env
import itertools
...
def main():
    ...
    for agent in range(NUM_AGENTS):
        net_env.set_video_chunk(DEFAULT_QUALITY, False, None, agent)

    while True:  # serve video forever
        
        end = net_env.step()
        
        action_space = net_env.get_action()
        
        for agent, action in enumerate(action_space):
            if action is False:
                continue
            ...
            end_of_video, video_chunk_remain, bit_rate, is_handover, new_sat_id= \
                net_env.get_result(agent, model_type=MPC_TYPE)
            ...
            net_env.set_video_chunk(bit_rate, is_handover, new_sat_id, agent)
            # hack
        ...
        if end:
            ...
            net_env.reset()
            for agent in range(NUM_AGENTS):
                net_env.set_video_chunk(bit_rate[agent], False, None, agent)
