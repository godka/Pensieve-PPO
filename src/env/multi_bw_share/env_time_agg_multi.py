...
from util.constants import DEFAULT_QUALITY, REBUF_PENALTY, SMOOTH_PENALTY, VIDEO_BIT_RATE, BUFFER_NORM_FACTOR, \
    BITRATE_WEIGHT, CHUNK_TIL_VIDEO_END_CAP, M_IN_K, PAST_TEST_LEN, A_DIM, PAST_LEN, BITRATE_REWARD, PAST_SAT_LOG_LEN, A_SAT
from util.encode_multi import encode_other_sat_info
from . import core_cent_time_multi as abrenv
...
class ABREnv():
    def __init__(self, random_seed=RANDOM_SEED, num_agents=NUM_AGENTS, reward_func=REWARD_FUNC, train_traces=None):
        ...
        global S_INFO
        S_INFO = (10 + 8 * (self.num_users - 1) + 1 + (self.num_users - 1) * PAST_SAT_LOG_LEN) + A_SAT - 2
    ...
    def reset_agent(self, agent):
        ...
        state[10 + 8 * (self.num_users - 1) + 1:(10 + 8 * (self.num_users - 1) + 1 + (self.num_users - 1) * PAST_SAT_LOG_LEN),
        0:A_SAT] = np.reshape(other_user_sat_decisions, (-1, A_SAT))

        state[(10 + 8 * (self.num_users - 1) + 1 + (self.num_users - 1) * PAST_SAT_LOG_LEN):((10 + 8 * (self.num_users - 1) + 1 + (self.num_users - 1) * PAST_SAT_LOG_LEN) + A_SAT - 2), :PAST_LEN] = np.array(other_sat_bws) / 10
    ...
    def set_sat(self, agent, sat):
        if sat == 0:
            ...
            if len(self.other_ids[agent]) <= sat-2 or sat == 0:
                return False

            sat = self.other_ids[agent][sat-2]
    ...
    def step(self, action, agent):
        ...
        state[
        10 + 8 * (self.num_users - 1) + 1:(10 + 8 * (self.num_users - 1) + 1 + (self.num_users - 1) * PAST_SAT_LOG_LEN),
        0:A_SAT] = np.reshape(other_user_sat_decisions, (-1, A_SAT))
