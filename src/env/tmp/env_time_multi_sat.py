...
from util.constants import DEFAULT_QUALITY, REBUF_PENALTY, SMOOTH_PENALTY, VIDEO_BIT_RATE, BUFFER_NORM_FACTOR, \
    BITRATE_WEIGHT, CHUNK_TIL_VIDEO_END_CAP, M_IN_K, PAST_LEN, A_DIM, PAST_LEN, BITRATE_REWARD, MAX_SAT, MAX_SAT
from . import core_time as abrenv
from . import load_trace as load_trace

# bit_rate, buffer_size, next_chunk_size, bandwidth_measurement(throughput and time), chunk_til_video_end
S_INFO = 6 + 3 + MAX_SAT - 2
...
class ABREnv():
    ...
    def reset_agent(self, agent):
        ...
        if connected_time and
        state[8, :2] = [float(connected_time[0]) / BUFFER_NORM_FACTOR / 10,
