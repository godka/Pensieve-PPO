...
from util.constants import CHUNK_TIL_VIDEO_END_CAP, BUFFER_NORM_FACTOR, VIDEO_BIT_RATE, REBUF_PENALTY, SMOOTH_PENALTY, \
    DEFAULT_QUALITY, BITRATE_WEIGHT, M_IN_K, A_DIM, PAST_TEST_LEN, PAST_LEN, BITRATE_REWARD, TEST_TRACES, PAST_SAT_LOG_LEN
    ...
    with tf.Session() as sess:
        ...
        while True:  # serve video forever
            ...
            for i in range(USERS):
                ...
                if is_handover:
                    state[agent][8 + 9 * i, 0:PAST_TEST_LEN] = np.zeros((1, PAST_TEST_LEN))
                    state[agent][9 + 9 * i, 0:PAST_TEST_LEN] = np.zeros((1, PAST_TEST_LEN))
