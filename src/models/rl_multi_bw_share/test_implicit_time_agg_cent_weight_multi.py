...
from util.constants import CHUNK_TIL_VIDEO_END_CAP, BUFFER_NORM_FACTOR, VIDEO_BIT_RATE, REBUF_PENALTY, SMOOTH_PENALTY, \
    DEFAULT_QUALITY, BITRATE_WEIGHT, M_IN_K, A_DIM, PAST_TEST_LEN, PAST_LEN, BITRATE_REWARD, TEST_TRACES, PAST_SAT_LOG_LEN, A_SAT
from util.encode_multi import encode_other_sat_info, one_hot_encode

os.environ['CUDA_VISIBLE_DEVICES'] = '-1'
import numpy as np
import tensorflow.compat.v1 as tf
from env.multi_bw_share import fixed_env_cent_time_multi as env
from env.multi_bw_share import load_trace as load_trace
from models.rl_multi_bw_share.ppo_spec import ppo_implicit_agg_weighted_multi as network
import structlog
import logging
...
SUMMARY_DIR = './test_results_imp_agg_weight_multi' + str(USERS)
S_INFO = 10 + 8 * (USERS - 1) + 1 + (USERS - 1) * PAST_SAT_LOG_LEN + A_SAT - 2
...
def main():
    ...
    with tf.Session() as sess:
        ...
        while True:  # serve video forever
            ...
            if is_handover:
                state[agent][8, 0:PAST_TEST_LEN] = np.zeros((1, PAST_TEST_LEN))
                state[agent][9, 0:PAST_TEST_LEN] = np.zeros((1, PAST_TEST_LEN))

            state[agent][8, -1] = np.array(cur_sat_user_num) / 10
            state[agent][9, -1] = np.array(next_sat_user_num) / 10
            if prev_connected_time[agent]:
                state[agent][10, :2] = [float(prev_connected_time[agent][0]) / BUFFER_NORM_FACTOR / 10,
                                                float(prev_connected_time[agent][1]) / BUFFER_NORM_FACTOR / 10]
            else:
                state[agent][10, :2] = [0, 0]
            i = 0
            ...
            if next_sat_ids is not None:
                next_sat_id = next_sat_ids[agent]
            other_ids, other_user_sat_decisions, other_sat_num_users, other_sat_bws, cur_user_sat_decisions \
            ...
            state[agent][10 + 8 * (USERS - 1) + 1:(
                        10 + 8 * (USERS - 1) + 1 + (USERS - 1) * PAST_SAT_LOG_LEN),
            0:A_SAT] = np.reshape(other_user_sat_decisions, (-1, A_SAT))

            state[agent][(10 + 8 * (USERS - 1) + 1 + (USERS - 1) * PAST_SAT_LOG_LEN):(
                        (10 + 8 * (USERS - 1) + 1 + (USERS - 1) * PAST_SAT_LOG_LEN) + A_SAT - 2),
            :PAST_LEN] = np.array(other_sat_bws) / 10
            ...
            if not end_of_video:
                if sat[agent] == 1 or (sat[agent] != 0 and len(prev_other_ids[agent]) > sat[agent] - 2):
                    if sat[agent] != 1:
                        sat[agent] = prev_other_ids[agent][sat[agent] - 2]
