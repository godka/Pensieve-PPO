...
from env.multi_bw_share.env_time_agg_multi import ABREnv
from models.rl_multi_bw_share.ppo_spec import ppo_implicit_agg_weighted_multi as network
import tensorflow.compat.v1 as tf
import structlog
from util.constants import A_DIM, NUM_AGENTS, TRAIN_TRACES, PAST_SAT_LOG_LEN, A_SAT
import logging
...
RANDOM_SEED = 42
SUMMARY_DIR = './ppo_imp_agg_weight_multi'
MODEL_DIR = '..'

TEST_LOG_FOLDER = './test_results_imp_agg_weight_multi'
PPO_TRAINING_EPO = 5
...
USERS = args.user
# A_SAT = USERS + 1
S_DIM = [10 + 8 * (USERS - 1) + 1 + (USERS - 1) * PAST_SAT_LOG_LEN + A_SAT - 2, 8]
...
def testing(epoch, nn_model, log_file):
    ...
    # run test script
    log.info('python test_implicit_time_agg_cent_weight_multi.py ', nn_model=nn_model + ' ' + str(USERS))
    os.system('python test_implicit_time_agg_cent_weight_multi.py ' + nn_model + ' ' + str(USERS))
