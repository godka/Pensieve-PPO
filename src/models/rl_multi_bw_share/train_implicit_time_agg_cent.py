...
USERS = args.user
# A_SAT = USERS + 1
S_DIM = [8 + 9 * (USERS - 1) + 1 + (USERS - 1) * PAST_SAT_LOG_LEN, 8]
...
def testing(epoch, nn_model, log_file):
    ...
    # run test script
    log.info('python test_implicit_time_agg_cent.py ', nn_model=nn_model + ' ' + str(USERS))
    os.system('python test_implicit_time_agg_cent.py ' + nn_model + ' ' + str(USERS))
