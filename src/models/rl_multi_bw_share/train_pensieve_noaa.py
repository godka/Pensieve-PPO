...
from env.multi_bw_share.env_pensieve_noaa_time import ABREnv
...
RANDOM_SEED = 42
SUMMARY_DIR = './pensieve_noaa'
MODEL_DIR = '..'
TEST_LOG_FOLDER = './test_results_pensieve_noaa'
PPO_TRAINING_EPO = 5
...
def testing(epoch, nn_model, log_file):
    ...
    # run test script
    log.info('python test_pensieve_noaa.py ', nn_model=nn_model + ' ' + str(USERS) + ' ' + HO_TYPE)
    os.system('python test_pensieve_noaa.py ' + nn_model + ' ' + str(USERS) + ' ' + HO_TYPE)
...
def agent(agent_id, net_params_queue, exp_queue):
    env = ABREnv(agent_id, num_agents=USERS, ho_type=HO_TYPE, reward_func=REWARD_FUNC, train_traces=TRAIN_NOAA_TRACES)
