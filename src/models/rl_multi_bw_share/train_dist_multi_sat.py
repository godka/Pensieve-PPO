...
from env.multi_bw_share.env_time_multi_sat import ABREnv
from models.rl_multi_bw_share.ppo_spec import ppo_implicit_multi_sat as network
import tensorflow.compat.v1 as tf
import structlog
from util.constants import A_DIM, TRAIN_TRACES, MAX_SAT
import logging

os.environ['CUDA_VISIBLE_DEVICES'] = '-1'
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'

S_DIM = [6 + 3 + MAX_SAT - 2, 8]
ACTOR_LR_RATE = 1e-4
...
def testing(epoch, nn_model, log_file):
    ...
    # run test script
    log.info('python test_dist_multi_sat.py ', nn_model=nn_model + ' ' + str(USERS))
    os.system('python test_dist_multi_sat.py ' + nn_model + ' ' + str(USERS))
